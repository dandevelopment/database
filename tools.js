...
if(getLogin() != "true"&& getCookie('bypasn') == ''&& document.location.href.split('/')[Number(document.location.href.split('/').length) - 1] == 'index.html' || document.location.href.split('/')[Number(document.location.href.split('/').length) - 1] == ''){
...
var checkForAchievment = setInterval(function () {
	...
	//for all achievments
		...
		//only achievments inside of law enforcement training
...
setTimeout(function(){notify(message)},6000)
	...
	var audioo = ''
	if(getCookie('soundO') != 'false'){
	audioo = '<audio controls autoplay style="display:none;">  <source src="awareness.ogg" type="audio/ogg"></audio>'
	}
    getit('messageNow').innerHTML = '<div id="messageNow" style="position:absolute;bottom:0px;right:0;">' + audioo + '<div style="width:150px;background-color:#3a3a3a;border-radius:10px;color:white;boxShadow:0 8px 16px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);padding-top:20px;padding-bottom:20px;padding-left:20px;padding-right:20px;margin-right:10px;margin-bottom:10px;"><p style="">' + message + '</p></div>';
 ...
 */
     ...
     setCookie('AWARD_law enforcement training', '60|Recognizes the accumulation of 60 hours of training in decided areas.|AWARD')
    setCookie('AWARD_community service', '100|Acknowledges 100 hours of community service, including crowd traffic control, parking service, community events, etc. Service must be preapproved and logged by the Advisor.|AWARD')
    setCookie('AWARD_crime prevention', '25|Includes the basic 8 hours of crime prevention instruction as well as an additional 8 hours of training in areas such as neighborhood watch, home security survey, and bicycle registration. The award is presented to Explorers who participate in at least three departmental crime prevention projects with a total of 25 hours of activity.|AWARD')
    ...
    setCookie('AWARD_perfect attendance', '1|Recognizes attendance at each scheduled meeting of the post during the preceding year.|AWARD')
